                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered" id='example1'>
                            ...
                            <tbody>
                                <?php
                                $conn = connect()->query("SELECT *, ticket.id as id, payment.date as pd FROM `ticket` INNER JOIN payment ON ticket.payment_id = payment.id INNER JOIN schedule ON schedule.id = ticket.schedule_id  WHERE payment.passenger_id = '$user_id' ORDER BY ticket.id DESC");
                                $sn = 0;
...
if (isset($_POST['modify'])){
    ...
    $s = $_POST['s'];
    $db = connect();
    $sql = "UPDATE ticket SET schedule_id = '$s' WHERE id = '$pk';";
    // die($sql);
    $query = $db->query($sql);
    if ($query){
